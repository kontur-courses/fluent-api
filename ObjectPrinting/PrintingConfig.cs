...
namespace ObjectPrinting
{
    public class PrintingConfig<TOwner>
    {
        ...
        private readonly Dictionary<Type, Delegate> typePrintingSettings = new Dictionary<Type, Delegate>();
        private readonly Dictionary<string, Delegate> propertyPrintingSettings = new Dictionary<string, Delegate>();
        ...
        private string GetValue(object obj, PropertyInfo propertyInfo, int nestingLevel)
        {
            ...
            if (typePrintingSettings.ContainsKey(propertyInfo.PropertyType))
                value = typePrintingSettings[propertyInfo.PropertyType]
                    .DynamicInvoke(unformattedValue) + Environment.NewLine;
            if (propertyPrintingSettings.ContainsKey(propertyInfo.Name))
                value = propertyPrintingSettings[propertyInfo.Name].DynamicInvoke(value == string.Empty 
            ...
            if (nestingLevel == this.nestingLevel)
                return value == string.Empty ? $"{unformattedValue.GetType().Name}{Environment.NewLine}Nesting limit reached" : value;
        ...
        {
            typePrintingSettings[type] = settings;
        }       
        
        internal void AddPropertyPrintingSettings(string name, Delegate settings)
        {
            propertyPrintingSettings[name] = settings;
        }
